---
title: "Poor Economics: A Radical Rethinking of The Way To Fight Global Poverty"
author: "Abhijit V. Banerjee, Esther Duflo"
year_read: 2017
rating: 3
quake: false
tags:
  - "people-over-politics"
last_imported: "2026-06-26"
key_takeaways_status: "written"
---

## Key Takeaways

<!-- key-takeaways -->
_No highlights captured in `raw/books/` for this book. Metadata is from `raw/books.json`. Add a Readwise export to `raw/books/poor-economics-a-radical-rethinking-of-the-way-to-fight-global-poverty.md` and re-ingest, or write takeaways manually._
<!-- /key-takeaways -->

## Connections

<!-- interconnections -->
- [Utopia For Realists](/books/utopia-for-realists) — [Bregman](/notes/rutger-bregman) draws directly on [Esther Duflo](/notes/esther-duflo)'s RCT work (the Hyderabad microcredit study) to argue cash transfers beat microcredit; this book is the evidentiary backbone of his anti-poverty chapter.
<!-- /interconnections -->

## Highlights

_No source highlights captured._
