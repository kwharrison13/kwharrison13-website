---
title: "Vertical Integration"
type: "concept"
publish: false
confidence: "low"
created: "2026-06-17"
updated: "2026-06-19"
last_updated_by: "agent"
sources:
  - "Roam Research daily-note export — September 22nd, 2021 (Index Ventures sourcing/diligence call log): ~/Downloads/September 22nd, 2021.md"
tags:
  - "strategy"
  - "business-model"
  - "healthcare"
aliases:
  - "Vertical Integration"
related:
  - "[[September 22nd, 2021 — Adam Chekroud (Spring Health)]]"
  - "[[Spring Health]]"
  - "[[Behavioral Health]]"
  - "[[Adam Chekroud]]"
---

# Vertical Integration

**One-line definition:** *Owning the full care-delivery stack to capture the value of the transaction; [Spring Health](/notes/spring-health)'s core strategy in behavioral health.*

## How sources describe it

- [Spring Health](/notes/spring-health) frames going deeper (medication, substance-abuse, residential care, even high-cost patients) as vertical integration — "why give away that revenue?"; if you're not vertically integrated it's hard to capture the value of the transaction. (September 22nd, 2021 — Adam Chekroud (Spring Health))

## Where it shows up

- September 22nd, 2021 — Adam Chekroud (Spring Health) — Spring Health's vertical-integration thesis.

## Related concepts

- [Spring Health](/notes/spring-health) — exemplar; frames going deeper into medication, substance-abuse, and residential care as vertical integration.
- [Behavioral Health](/notes/behavioral-health) — its market.
- [Adam Chekroud](/notes/adam-chekroud) — Spring Health founder who articulated the "why give away that revenue?" vertical-integration thesis.
