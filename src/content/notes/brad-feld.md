---
title: "Brad Feld"
type: "person"
publish: false
confidence: "medium"
created: "2026-06-17"
updated: "2026-06-25"
last_updated_by: "agent"
sources:
  - "wiki/essays-research/Historical Futurism - Research.md"
  - "wiki/long-reads/How To Predict The Future.md"
  - "wiki/essays-research/The State of Startup Media - Research.md"
tags: []
aliases:
  - "Brad Feld"
  - "Feld Notes"
  - "Feld Thoughts"
related:
  - "[[How To Predict The Future]]"
  - "[[Historical Futurism]]"
  - "[[Forecasting]]"
  - "[[The State of Startup Media - Research]]"
reference_count: 3
---

# Brad Feld

A venture capitalist and prolific writer who recurs in Kyle's notes mainly as the author of [How To Predict The Future](/notes/how-to-predict-the-future) (feld.com, 2012) — a long read Kyle annotated. In that piece Feld lays out his core forecasting method: use the smooth, *linear* trend in underlying hardware to time the *non-linear* technology disruptions that ride on top of it ("forget about trying to predict advancements in software and just look at the hardware trend"). That essay anchors Feld's place in Kyle's [Historical Futurism](/notes/historical-futurism) and [Forecasting](/notes/forecasting) threads (Historical Futurism - Research).

Feld also appears in Kyle's media-landscape thinking: his long-running blog (cited as "Feld Notes" / Feld Thoughts) is listed alongside [Stratechery](/notes/stratechery), Exponential View, and [Gates Notes](/notes/bill-gates) as an example of an influential operator-written publication (The State of Startup Media - Research).

**Context:** Brad Feld is a Boulder-based venture capitalist, co-founder of Foundry Group and Techstars, and an early Mobius Venture Capital partner. He is a widely read startup blogger (Feld Thoughts) and co-author of "Venture Deals."

## Where this appears

- [How To Predict The Future](/notes/how-to-predict-the-future) — author of the annotated essay on timing tech disruption via linear hardware trends.
- Historical Futurism - Research — listed among the people/sources feeding Kyle's historical-futurism research; the Feld essay is a key forecasting source.
- The State of Startup Media - Research — his blog cited as an operator-written publication in the startup-media landscape.
